...
- 課程(`/courses`,支援關鍵字搜尋):累積 VP 達 Mentor 門檻才能開課,報名要付點數(`Course.pointsCost`,每堂課自訂);標記為「基礎課程」的定價建議是新手禮包的一半,讓新用戶剛好夠報名 2 堂 —— 不是另外做一個「免費名額」欄位,單純是定價的自然結果;每堂課都有開放式討論區(`CourseComment`,任何登入使用者都能發問),`/me` 會列出自己報名過的課程
- 讀書會 / 學習小隊(`/projects?studyGroup=1`):不是獨立模組,而是 `Project` 多加 `isStudyGroup`/`capacity` 兩個欄位 —— 沿用既有的成員、角色、建議討論串、進度動態機制,`capacity` 額滿後新成員無法加入,結案時發起人會額外拿到讀書會完成獎勵(`STUDY_GROUP_COMPLETED`,+200 VP)
- 後台(`/sys`):`User.isAdmin` 欄位控管,管理員可以互相授予管理員身份、手動調整任何人的點數、強制結案或刪除專案、刪除不當的建議與留言

尚未實作:測試任務協作、競賽模組、簽到/招生、更完整的課程內容編排(目前課程只有單一說明欄位)。下方「產品願景」是更完整的長期規劃。
...
| **Community｜讀書會** | 不做傳統論壇,而是「學習小隊」:6～10 人一組,每週一個主題 → 實作 → 分享 Demo → Code Review → 點數獎勵 | 已有地基(`Project.isStudyGroup`/`capacity`,結案發完成獎勵,重用既有 Project/角色模型,還沒有「每週主題」的結構化排程) |
| **Teach｜開課** | 把 VibeHub 從「學習平台」變成人才生態系的關鍵層:學習者 → 做專案 → 累積作品 → 成為 Mentor → 開讀書會 → 開課 → 成為講師 | 已有地基(Mentor 門檻 + 開課/報名/計價/課程 Q&A 已上線,讀書會也已上線) |
...
「花」的部分已經有第一個真實場景:報名課程要付點數(`/courses`)。其他花費項目(工作坊、AI API 額度、Mentor 諮詢等)尚未開始。「賺」的部分,「辦讀書會 +200」也已經是真實機制,不再只是規劃(讀書會結案時發起人自動入帳)。
